...
public class RobotControlRevisedA implements Control
{
	private Robot robot;
...
//				{3, 2, 2, 3, 1, 1 };
	...
	{
		String prompt = String.format("Enter number of bars(min %d/max %d):", Control.MIN_BARS, Control.MAX_BARS);
		int barsNumber=nextIntValidatedInRange(prompt, Control.MIN_BARS, Control.MAX_BARS, sc);
		int bar[]=new int [barsNumber];
		for(int i=0; i<bar.length; i++)
		{
			String insBars=String.format("Enter height of bar %d of %d (min %d/max %d): ", i+1, bar.length, Control.MIN_BAR_HEIGHT, Control.MAX_BAR_HEIGHT);
			bar[i]=nextIntValidatedInRange(insBars, Control.MIN_BAR_HEIGHT, Control.MAX_BAR_HEIGHT, sc);
		}
		return bar;
	}
	...
	private int[] getBlockHeights(Scanner sc)
	{
		...
		String prompt = String.format("Enter number of blocks(min %d/max %d):", MIN_BLOCKS, MAX_BLOCKS);
		int blocksNumber=nextIntValidatedInRange(prompt, MIN_BLOCKS, MAX_BLOCKS, sc);
		int block[]=new int [blocksNumber];
		int sum=0;
		for(int i=0;i<block.length;i++)
		{
			String insBlocks=String.format("Enter height of block %d of %d (min %d/max %d): ", i+1, block.length, MIN_BLOCKS, MAX_BLOCKS);
			block[i]=nextIntValidatedInRange(insBlocks, Control.MIN_BLOCK_HEIGHT, Control.MAX_BLOCK_HEIGHT, sc);
	...
	private int nextIntValidatedInRange(String prompt, int min, int max, Scanner sc)
	{
		int num=0;
		boolean flag=true;
		while(flag)
